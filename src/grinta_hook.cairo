...
pub mod GrintaHook {
    ...
    // Market price sanity bounds (WAD)
    // GRIT is a stablecoin targeting $1. Reject prices outside [$0.001, $1000]
    // to prevent garbage delta prices from crashing the PID controller.
    const MIN_MARKET_PRICE: u256 = 1_000_000_000_000_000; // 0.001 WAD ($0.001)
    const MAX_MARKET_PRICE: u256 = 1_000_000_000_000_000_000_000; // 1000 WAD ($1000)
    ...
    impl ExtensionImpl of grinta::interfaces::igrinta_hook::IExtension<ContractState> {
        ...
        fn after_swap(
            ...
            // Only accept price if it's within sane bounds [$0.001, $1000]
            // This prevents garbage prices from thin liquidity from crashing the system
    ...
    impl GrintaHookImpl of grinta::interfaces::igrinta_hook::IGrintaHook<ContractState> {
        ...
        fn set_market_price(ref self: ContractState, price: u256) {
            assert(price >= MIN_MARKET_PRICE, 'HOOK: price too low');
            assert(price <= MAX_MARKET_PRICE, 'HOOK: price too high');
            self.last_market_price.write(price);
            let now = get_block_timestamp();
            self.emit(MarketPriceUpdated { market_price: price, timestamp: now });
        }
